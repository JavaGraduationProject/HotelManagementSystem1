  ...
  <el-card class="box-card" shadow="always">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="退房时间">
        <template slot-scope="scope">
          {{ scope.row.orderStatus==3?(scope.row.updateTime|formatDateV2):''}}
        </template>
      </el-table-column>
      <el-table-column
        label="修改时间|创建时间">
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      getAllOrder(this.searchData).then(response => {
    ...
    handleEdit(index, row) {
      this.$router.push({ path: '/order/edit', query: { data: JSON.stringify(row) }})
    },
