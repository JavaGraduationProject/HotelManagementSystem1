<template>
  <el-card class="box-card" shadow="always">
    <div slot="header">
      <span>角色管理</span>
      <el-input v-model="searchData" placeholder="请输入角色名称">
    ...
    <div style="padding: 14px;">
      <div class="bottom">
        <el-button type="primary" @click="navigateTo('add')">添加角色信息</el-button>
      </div>
    </div>
  </el-card>
</template>
...
<script>
import { getAllWorkRole, delWorkRole } from '../../api/workRole'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getAllWorkRole(this.searchData).then(response => {
    ...
    navigateTo(val) {
      this.$router.push({ path: '/workRole/' + val })
    },
    ...
    handleDel(row) {
      row.visible2 = false
      row.loading = true
      delWorkRole(row.id).then(response => {
        const res = response
        if (res.code === 1000) {
          this.$message({
            message: '删除成功！',
